...
namespace LibraryEditor
{
    public partial class LMain : Form
    {
        ...
        private MLibraryV2.MImage _selectedImage, _exportImage, _editImage;
        ...
        private bool MobSelected = false;
        ...
        public LMain()
        {
            ...
            this.AllowDrop = true;
            this.DragEnter += new DragEventHandler(LiveInfoForm_DragEnter);
            this.DragDrop += new DragEventHandler(LiveInfoForm_DragDrop);
        ...
        private void LiveInfoForm_DragDrop(object sender, DragEventArgs e)
        {
        ...
        private void LiveInfoForm_DragEnter(object sender, DragEventArgs e)
        {
        ...
        private void PreviewListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (LockOffsetChkBox.Checked)
            {
                if (MobChkBox.Checked)
                {
                    ImageBox.Location = new Point(295 + _selectedImage.X, 392 + _selectedImage.Y);
                }
                else
                {
                    ImageBox.Location = new Point(_selectedImage.X, _selectedImage.Y);
                }
            }
            else
            {
                ImageBox.Location = new Point(58, 23);
            }
            if (ViewMode == "Image")
            {
                if (BackgroundPicBox1.Visible)
                {
                    ImageBox.Parent = BackgroundPicBox1;
        ...
        private void ExportButton_Click(object sender, EventArgs e)
        {
            ...
            System.Windows.Forms.ListView.SelectedIndexCollection _col = PreviewListView.SelectedIndices;
        ...
        #region Previous/Next Image Buttons
        ...
        {
            using (FolderBrowserDialog fbd = new FolderBrowserDialog() { Description = "Select your Client Folder." })
            {
        ...
        #region Load Data Directory (TreeView)
        ...
        private void TreeAfterSelect()
        {
            ...
            if (TreeBrowser.SelectedNode.FullPath.ToString().Contains(".lib") || TreeBrowser.SelectedNode.FullPath.ToString().Contains(".Lib"))
            {
            ...
            {
                MessageBox.Show("File must be in '.Lib' format.");
            }
        }

        #region Mob Background Button
        private void MobBackgroundPicBox_Click(object sender, EventArgs e)
        {
            if (BackgroundPicBox1.Visible)
            {
                BackgroundPicBox1.Visible = false;
                ImageBox.Parent = panel;
                MobChkBox.Checked = false;
                LockOffsetChkBox.Checked = false;
            }
            else
            {
                BackgroundPicBox1.Visible = true;
                MobChkBox.Checked = true;
                LockOffsetChkBox.Checked = true;
                ImageBox.Parent = BackgroundPicBox1;
            }
        }
        ...
        #region Offset Buttons

        private void BtnXMinusTen_Click(object sender, EventArgs e)
        {
            DecreaseX(10);
        }

        private void BtnXMinusFive_Click(object sender, EventArgs e)
        {
            DecreaseX(5);
        }

        private void BtnXMinusOne_Click(object sender, EventArgs e)
        {
            DecreaseX(1);
        }

        private void BtnYMinusTen_Click(object sender, EventArgs e)
        {
            DecreaseY(10);
        }

        private void BtnYMinusFive_Click(object sender, EventArgs e)
        {
            DecreaseY(5);
        }

        private void BtnYMinusOne_Click(object sender, EventArgs e)
        {
            DecreaseY(1);
        }

        private void BtnXPlusOne_Click(object sender, EventArgs e)
        {
            IncreaseX(1);
        }

        private void BtnXPlusFive_Click(object sender, EventArgs e)
        {
            IncreaseX(5);
        }

        private void BtnXPlusTen_Click(object sender, EventArgs e)
        {
            IncreaseX(10);
        }

        private void BtnYPlusOne_Click(object sender, EventArgs e)
        {
            IncreaseY(1);
        }
        private void BtnYPlusFive_Click(object sender, EventArgs e)
        {
            IncreaseY(5);
        }
        private void BtnYPlusTen_Click(object sender, EventArgs e)
        {
            IncreaseY(10);
        }

        private void IncreaseY(short IncreaseAmount)
        {
            if (_library == null) return;
            if (_library.FileName == null) return;
            if (PreviewListView.SelectedIndices.Count == 0) return;
            ...
            for (int i = _col[0]; i < (_col[0] + _col.Count); i++)
            {
                _editImage = _library.GetMImage(i);
                if (_editImage == null) return;

                short ImageY = _editImage.Y;
                _editImage.Y = (short)(ImageY + IncreaseAmount);
                numericUpDownY.Text = _selectedImage.Y.ToString();
                if (LockOffsetChkBox.Checked)
                {
                    if (MobChkBox.Checked)
                    {
        ...
        private void IncreaseX(short IncreaseAmount)
        {
            if (_library == null) return;
            if (_library.FileName == null) return;
            if (PreviewListView.SelectedIndices.Count == 0) return;
            ...
            for (int i = _col[0]; i < (_col[0] + _col.Count); i++)
            {
                ...
                short ImageX = _editImage.X;
                _editImage.X = (short)(ImageX + IncreaseAmount);
                numericUpDownX.Text = _selectedImage.X.ToString();
                if (LockOffsetChkBox.Checked)
                {
                    if (MobChkBox.Checked)
                    {
        ...
        private void DecreaseY(short DecreaseAmount)
        {
            if (_library == null) return;
            if (_library.FileName == null) return;
            if (PreviewListView.SelectedIndices.Count == 0) return;
            ...
            for (int i = _col[0]; i < (_col[0] + _col.Count); i++)
            {
                ...
                short ImageY = _editImage.Y;
                _editImage.Y = (short)(ImageY - DecreaseAmount);
                numericUpDownY.Text = _selectedImage.Y.ToString();
                if (LockOffsetChkBox.Checked)
                {
                    if (MobChkBox.Checked)
        ...
        private void DecreaseX(short DecreaseAmount)
        {
            if (_library == null) return;
            if (_library.FileName == null) return;
            if (PreviewListView.SelectedIndices.Count == 0) return;
            ...
            for (int i = _col[0]; i < (_col[0] + _col.Count); i++)
            {
                ...
                short ImageX = _editImage.X;
                _editImage.X = (short)(ImageX - DecreaseAmount);
                numericUpDownX.Text = _selectedImage.X.ToString();
                if (LockOffsetChkBox.Checked)
                {
                    if (MobChkBox.Checked)
