    ...
    public partial class PlayerInfoForm : Form
    {
        ...
        private void UpdatePetInfo()
        {
            ...
            foreach (MonsterObject Pet in Character.Player.Pets)
            {
                ...
                listItem.SubItems.Add($"{Pet.Stats[Stat.MinAC]}-{Pet.Stats[Stat.MaxAC]}");
                listItem.SubItems.Add($"{Pet.Stats[Stat.MinMAC]}-{Pet.Stats[Stat.MaxMAC]}");
                listItem.SubItems.Add($"{Pet.Stats[Stat.MinDC]}-{Pet.Stats[Stat.MaxDC]}");
        ...
        private void UpdatePlayerItems()
        {
            ...
            for (int i = 0; i < Character.Inventory.Length; i++)
            {
                ...
                {
                    inventoryItemListItem.SubItems.Add($"Inventory Bag | Slot: [{i - 5}]");
                }
            ...
            for (int i = 0; i < Character.AccountInfo.Storage.Length; i++)
            {
                ...
                if (i < 80)
                {
                    storeItemListItem.SubItems.Add($"Storage | Slot: [{i + 1}]");
                }
        ...
        private void UpdateButton_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to Update?", "Are you sure buddy?.", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning) != DialogResult.Yes) return;
