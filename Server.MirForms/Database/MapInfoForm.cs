...
namespace Server
{
    public partial class MapInfoForm : Form
    {
        ...
        public Envir PlayerEnvir => SMain.Envir;
        ...
        private string selectedDirectory; // Field for storing the selected directory path
        public MapInfoForm()
        {
            InitializeComponent();
            ...
            selectedDirectory = Properties.Settings.Default.selectedDirectory;
        ...
        private void UpdateInterface()
        {
            ...
            UpdatePlayersViewInterface();
            UpdateMonstersViewInterface();
        }
        ...
        private void UpdateMonstersViewInterface()
        {
            if (_selectedMapInfos.Count != 1)
            {
                monsterTreeView.Nodes.Clear();
                return;
            }
            if (_info != _selectedMapInfos[0])
            {
            ...
            List<MapObject> monsters = new List<MapObject>();

            foreach (MapObject obj in PlayerEnvir.Objects)
            {
                if (obj == null) continue;
                if (obj.Race != ObjectType.Monster) continue;
                if (obj.Master != null) continue;

                monsters.Add(obj);
            }

            Dictionary<string, int> monsterCount = new Dictionary<string, int>();

            foreach (MapObject monster in monsters)
            {
                if (monster == null) continue;
                if (monster.CurrentMapIndex != _info.Index) continue;
                if (string.IsNullOrEmpty(monster.Name)) continue;

                // Update the count in the dictionary
                if (monsterCount.ContainsKey(monster.Name))
                {
                    monsterCount[monster.Name]++;
                }
                else
                {
                    monsterCount[monster.Name] = 1;
                }
            }

            foreach (var entry in monsterCount)
            {
                string nodeName = $"Name: {entry.Key}, Amount: {entry.Value}";
                TreeNode parentNode = monsterTreeView.Nodes.Add(nodeName);

                var matchingMonsters = monsters.Where(monster => monster.Name == entry.Key && monster.CurrentMapIndex == _info.Index);

                foreach (MapObject matchingMonster in matchingMonsters)
                {
                    TreeNode subNode = parentNode.Nodes.Add($"Name: {matchingMonster.Name},X: {matchingMonster.CurrentLocation.X}, Y: {matchingMonster.CurrentLocation.Y} | Health: {matchingMonster.Health}/{matchingMonster.MaxHealth}");

                    if (matchingMonster.Health == 0)
                    {
        ...
        private void UpdatePlayersViewInterface()
        {
            if (_selectedMapInfos.Count != 1)
            {
            ...
            foreach (PlayerObject player in PlayerEnvir.Players)
            {
                if (player == null || player.CurrentMapIndex != _info.Index)
                    continue;

                TreeNode playerNode = new TreeNode($"Name: {player.Name}");

                #region Location

                TreeNode locationNode = new TreeNode("Location");
                locationNode.Nodes.Add($"Map: {player.CurrentMap.Info.Title}, File: {player.CurrentMap.Info.FileName}");
                locationNode.Nodes.Add($"X: {player.CurrentLocation.X}, Y: {player.CurrentLocation.Y}");

                playerNode.Nodes.Add(locationNode);
                ...
                statsNode.Nodes.Add($"Health: {player.HP}/{player.MaxHealth}");
                statsNode.Nodes.Add($"Mana: {player.MP}/{player.Stats[Stat.MP]}");
                statsNode.Nodes.Add($"Level: {player.Level}");
                statsNode.Nodes.Add($"EXP: {string.Format("{0:#0.##%}", player.Experience / (double)player.MaxExperience)}");
                statsNode.Nodes.Add($"AC: {player.Stats[Stat.MinAC]}/{player.Stats[Stat.MaxAC]}");
                statsNode.Nodes.Add($"AMC: {player.Stats[Stat.MinMAC]}/{player.Stats[Stat.MaxMAC]}");
                statsNode.Nodes.Add($"DC: {player.Stats[Stat.MinDC]}/{player.Stats[Stat.MaxDC]}");
                statsNode.Nodes.Add($"MC: {player.Stats[Stat.MinMC]}/{player.Stats[Stat.MaxMC]}");
                statsNode.Nodes.Add($"SC: {player.Stats[Stat.MinSC]}/{player.Stats[Stat.MaxSC]}");
                statsNode.Nodes.Add($"Attack Speed: {player.Stats[Stat.AttackSpeed]}");
                statsNode.Nodes.Add($"ACC: {player.Stats[Stat.Accuracy]}");
                statsNode.Nodes.Add($"AGIL: {player.Stats[Stat.Agility]}");

                playerNode.Nodes.Add(statsNode);

                #endregion

                #region Pets

                List<MonsterObject> pets = new List<MonsterObject>();
                Dictionary<string, int> petCount = new Dictionary<string, int>();

                TreeNode petNode = new TreeNode("Pets");

                playerNode.Nodes.Add(petNode);

                foreach (MonsterObject pet in player.Pets)
                {
                    pets.Add(pet);

                    if (petCount.ContainsKey(pet.Name))
                    {
                        petCount[pet.Name]++;
                    }
                    else
                ...
                foreach (var entry in petCount)
                {
                    string nodeName = $"Name: {entry.Key}, Amount: {entry.Value}";
                    TreeNode parentNode = petNode.Nodes.Add(nodeName);

                    var matchingPets = pets.Where(pet => pet.Name == entry.Key);

                    foreach (MonsterObject matchingPet in matchingPets)
                    {
                        TreeNode subNode = parentNode.Nodes.Add($"Name: {matchingPet.Name}");

                        if (matchingPet.Health == 0)
                        {
                            subNode.ForeColor = Color.Red;
                        }

                        TreeNode LocationNode = subNode.Nodes.Add("Location");

                        TreeNode MapName = LocationNode.Nodes.Add($"Map: {matchingPet.CurrentMap.Info.Title}, File: {matchingPet.CurrentMap.Info.FileName}");

                        TreeNode Coords = LocationNode.Nodes.Add($"X: {matchingPet.CurrentLocation.X}, Y: {matchingPet.CurrentLocation.Y}");

                        TreeNode StatsNode = subNode.Nodes.Add("Stats");

                        TreeNode HealthNode = StatsNode.Nodes.Add($"Health: {matchingPet.Health}/{matchingPet.MaxHealth}");

                        TreeNode PetLevelNode = StatsNode.Nodes.Add($"Level: {matchingPet.PetLevel}");
                    }
                }
                ...
                playerTreeView.Nodes.Add(playerNode);
            }
        }
        ...
        {
            string imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "envir", "Previews", "Minimaps", miniMapValue + ".png");
        ...
        private void SelectDirectoryButton_Click(object sender, EventArgs e)
        {
            using (FolderBrowserDialog folderDialog = new FolderBrowserDialog())
            {
                if (folderDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    // Store the selected directory in application settings
        ...
        private void OpenProgramButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("You must first select your map viewer's directory.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            string executablePath = FindExecutableInDirectory(selectedDirectory);
            ...
            {
                MessageBox.Show("No executable file found in the selected directory.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private string FindExecutableInDirectory(string directory)
        {
            try
            {
                string[] executableExtensions = { ".exe" }; // Only search for .exe files

                foreach (string extension in executableExtensions)
                {
                    string[] executableFiles = Directory.GetFiles(directory, $"*{extension}");

                    if (executableFiles.Length > 0)
                    {
                        return executableFiles[0]; // Return the first executable found
                    }
                }

                return null; // No executable files found
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error searching for executable: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
        }
    }
